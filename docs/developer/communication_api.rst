...
expected replies from a remote function calculator. JSON formatting must
be used for all messages except strings and numbers.
...
* `bounds` (``string``) : An N x 2 JSON Array, where N is the number ofparameters, the first column is the lower bound of each parameter, and the second column is the upper bound of each parameter.
